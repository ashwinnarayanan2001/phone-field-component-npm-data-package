...
function obtainCountryFormatting(countryData) {
    ...
    countryData.forEach((countryObj, index) => {
        if (libphonenumberJS.isSupportedCountry(countryObj.countryAlphaCode)) {
            ...
            '408 233 3333'
            [3, 3, 4]
            let phoneNumber = libphonenumberJS.getExampleNumber(countryObj.countryAlphaCode, examples);
            let displayFormatting = phoneNumber.formatInternational().substring(`${countryObj.countryCode}`.length).split(" ").filter(Boolean).map(num => num.length);
...
function findCountriesByCode(countryData, countryCode) {
    let filteredData = countryData.filter(countryObj => 
...
function insertPopulationData(allData, populationData) {
    ...
    countryData.forEach((countryObj, index) => {
        if (populationData[countryObj.countryAlphaCode]) {
            countryData[index]["population"] = populationData[countryObj.countryAlphaCode];
        }
    })
...
exports.allCountries = insertPopulationData(obtainCountryFormatting(obtainCountryInformation(intlTelInput)), populationData());
